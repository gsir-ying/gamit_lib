...
c
c     Now accepts only RINEX formats
...
C MOD TAH 22078: Added newrecordflag to be consistent with code
C     removed newmessageflag which does not seem to be used.
      character*1 newrecordflag
      character*3 recordtype
      character*4 navtype
      character*20 buff20
...
c       value in free format.
        ...
        if(version.lt.2.0.or.version.gt.4.0) call report_stat('WARNING'
...
c             this label for version 3 only
...
c         assume version 4
          read(buff80,'(a1)') newrecordflag
          if ( newrecordflag.eq.'>' ) then
            read(buff80,'(2x,a3,5x,a4)') recordtype,navtype
          endif
        endif                  
