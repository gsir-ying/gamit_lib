...
c             power   : transmitted power in watts                 I*4
     ...
     .             svn_in        ! SVN string for satellite we are looking for
      ...
      character*10 cospar_id   ! ID not used in 
      character*6  SatCat      ! Catalog number; not used in 
      ...
      integer*8 itimdif     ! function to return time difference in seconds?
...
c MOD MAF 20230929: Added one day (86400 seconds) to test to pass case
c   of PRN starting in the middle of a day when the time requested is
c   the beginning of the day
c    .              itimdif(start,time).le.0 .and.
     .              itimdif(start,time).lt.86400 .and.
...
c Get the transmitter power ( SATELLITE/TX_POWER block)
     ...
     .         ioerr)
* MOD MAF 20240804: Added nominal power based on satellite (from igs_satellite_metadata.snx)
C         GPS BLOCK I/II/IIA
          if ( antbody_in.eq.'BLOCK_I' .or. antbody_in.eq.'BLOCK_II'
     .         .or. antbody_in.eq.'BLOCK_IIA' ) then
            power = 50
C         GPS BLOCK IIR/IIR-A/IIR-B
          elseif ( antbody_in.eq.'BLOCK_IIR-A' .or.
     .             antbody_in.eq.'BLOCK_IIR-B' ) then
            power = 60
C         GPS BLOCK IIR-M
          elseif ( antbody_in.eq.'BLOCK_IIR-M' ) then
            power = 145
C         GPS BLOCK IIF
          elseif ( antbody_in.eq.'BLOCK_IIF' ) then
C           NO M-CODE FOR SVN62/PRN25 STARTING 05APR2011; NANU 2011026
            if ( svn_in.eq.'G062' .and. ((iyr.eq.2011.and.iday.ge.95)
     .           .or. iyr.gt.2011) ) then
              power = 154
            else
              power = 240
...
C         elseif ( antbody_in.eq.'GLONASS' ) then
C         GLONASS-M
          elseif ( antbody_in.eq.'GLONASS-M' ) then
            if ( (svn_in.eq.'R720'.and.((iyr.eq.2019.and.iday.eq.94.and.
     .            ihr.ge.5).or.(iyr.eq.2019.and.iday.gt.94).or.
     .            iyr.gt.2019)) .or. (svn_in.eq.'R735'.and.((iyr.eq.2016
     .            .and.iday.eq.33.and.((ihr.eq.17.and.imin.ge.30).or.
     .            ihr.gt.17)).or.(iyr.eq.2016.and.iday.gt.33).or.
     .            iyr.gt.2016)) ) then
              power = 20
            elseif ( svn_in.eq.'R715' .or. svn_in.eq.'R721' .or.
     .               svn_in.eq.'R733' .or. svn_in.eq.'R734' .or.
     .               (svn_in.eq.'R735'.and.(iyr.lt.2016.or.(iyr.eq.2016
     .                .and.iday.lt.33).or.(iyr.eq.2016.and.iday.eq.33
     .                .and.(ihr.lt.17.or.(ihr.eq.17.and.imin.lt.30)))))
     .               .or. svn_in.eq.'R736' ) then
              power = 25
            elseif ( svn_in.eq.'R719' ) then
              power = 40
            elseif ( svn_in.eq.'R716' .or. svn_in.eq.'R720' ) then
              power = 60
            elseif ( svn_in.eq.'R717' .or. svn_in.eq.'R730' .or.
     .               svn_in.eq.'R732' ) then
              power = 65
            elseif ( svn_in.eq.'R854' ) then
              power = 70
            elseif ( svn_in.eq.'R731' .or. svn_in.eq.'R742' .or.
     .               svn_in.eq.'R743' .or. svn_in.eq.'R744' .or.
     .               svn_in.eq.'R745' .or. svn_in.eq.'R747' .or.
     .               svn_in.eq.'R851' .or. svn_in.eq.'R852' .or.
     .               svn_in.eq.'R853' .or. svn_in.eq.'R857' ) then
              power = 85
            elseif ( svn_in.eq.'R855' ) then
              power = 100
            elseif ( svn_in.eq.'R858' .or. svn_in.eq.'R859' .or.
     .               svn_in.eq.'R860' .or. svn_in.eq.'R861' ) then
              power = 110
            elseif ( svn_in.eq.'R856' ) then
              power = 120
            else
              power = 50
            endif
C         GLONASS-K1
          elseif ( antbody_in(1:10).eq.'GLONASS-K1' ) then
            if ( svn_in.eq.'R801' ) then
              power = 135
            else
              power = 105
            endif
...
C         GLONASS-K2
* MOD MAF 20240802: PRELIMINARY VALUE COPIED FROM HIGHEST PREVIOUS TX_POWER FOR GLONASS[-K1] IN
*                   IGS_SATELLITE_METADATA.SNX BUT LIKELY DIFFERENT WITH BROADCAST OF FDMA+CDMA!
          elseif ( antbody_in.eq.'GLONASS-K2' ) then
            power = 135
          else
