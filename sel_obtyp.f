...
* MOD MAF 220304: Added L2X and L2L between L2 and L2Y to accommodate observables reported by u-blox receivers
...
* MOD TAH 200511: Added L5 frequency choice (default L2)
* MOD MAF 220304: Added C2X and C2L between P2 and C2Y to accommodate observables reported by u-blox receivers
        if( gnsslf.eq.'G5' ) then   
...
*       no longer mixed.
* MOD MAF 220304: Added L7D between L7 and L7X to accommodate observables reported by many receivers
        if( gnsslf.eq.'C7' ) then   
           call fill_obtypx(2,'L7 ',nobtyp,rxobtyp,iobtypx)
           call fill_obtypx(2,'L7D',nobtyp,rxobtyp,iobtypx)  
...
*       change undoes 200223 mod above so that frequencies are
*       no longer mixed.
* MOD MAF 220304: Added C7D between C7 and C7X to accommodate observables reported by many receivers
        if( gnsslf.eq.'C7' ) then   
           call fill_obtypx(4,'C7 ',nobtyp,rxobtyp,iobtypx)
           call fill_obtypx(4,'C7D',nobtyp,rxobtyp,iobtypx)
