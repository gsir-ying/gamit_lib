...
c          and vice versa. The definitive place of  where all antennas and receivers
c           are listed is in a data file rcvant.dat in tables. The
...
* NOD TAH 031110: Addeded home_dir so that tables/rcvant.dat can be opened
...
!      if(ioerr.ne.0) then
!* MOD TAH 031110: See if we can open a copy through the /tables
...
!      end if
     ...
     .     ,'-- may need link to good/tables/rcvant.dat'
...
c get the full RINEX name from the code
...
cccccwanghutongji20190305
          if (indx .le. 0 ) goto 15 
cccccwanghutongji20190305
