...
c     modified by M. Floyd 16 Dec 21 for RINEX Version 4.00 (no changes, just warning)
...
c              check to see if different factors for different SVs  
               ...
               if( nw.ne.0 ) then  
c** rwk 190805: Since  won't support different wavelength factors for
...
c     check for reasonability
      if ( rxver.eq.1 .or. ( int(rxver).eq.2 .and. rxver.gt.2.20 )
     .    .or. ( int(rxver).eq.3 .and. rxver.gt.3.05 )
     .    .or. ( int(rxver).eq.4 .and. rxver.gt.4.00 ) ) then
         write (message,'(a,f9.2,a)') 'RINEX version = ',rxver
     .                    ,' not 1 or 2-2.20 or 3.00-3.05 or 4.00'
         call report_stat('WARNING',prog_name,'lib/rrxhed',fname
     .                   , message,0)
